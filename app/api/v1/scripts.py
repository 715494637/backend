...
router = APIRouter(prefix="/admin/scripts", tags=["话术库管理"])
...
@router.post("")
async def create_script(data: dict, current_admin: User = Depends(get_current_admin), db: AsyncSession = Depends(get_db)) -> Any:
    """创建话术（仅管理员）"""
    return await ScriptService.create(db, data)


@router.put("/{script_id}")
async def update_script(script_id: str, data: dict, current_admin: User = Depends(get_current_admin), db: AsyncSession = Depends(get_db)) -> Any:
    """更新话术（仅管理员）"""
...
async def delete_script(script_id: str, current_admin: User = Depends(get_current_admin), db: AsyncSession = Depends(get_db)):
    """删除话术（仅管理员）"""
