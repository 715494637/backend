...
from app.models import VipLevel as VipLevelModel, EnterpriseStats as EnterpriseStatsModel, ServiceRequest as ServiceRequestModel, User as UserModel, SpecialProject as SpecialProjectModel
...
class VipService:
    ...
    async def get_levels(db: AsyncSession) -> List[dict]:
        result = await db.execute(
            select(VipLevelModel).where(VipLevelModel.is_active == "1").order_by(VipLevelModel.sort_order)
        )
    ...
    async def create_level(db: AsyncSession, data: dict) -> dict:
        level = VipLevelModel(
            ...
            max_amount=str(data.get("max_amount", "")),
    ...
    async def get_stats(db: AsyncSession, enterprise_name: str) -> dict:
        result = await db.execute(select(EnterpriseStatsModel).where(EnterpriseStatsModel.enterprise_name == enterprise_name))
    ...
    async def select_projects(db: AsyncSession, user_id: str, project_ids: List[str]) -> bool:
        ...
        project_titles = {}
        if project_ids:
            proj_result = await db.execute(select(SpecialProjectModel).where(SpecialProjectModel.id.in_(project_ids)))
